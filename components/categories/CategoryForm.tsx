...
const CategoryForm: React.FC<CategoryFormProps> = ({ initialData }) => {
  ...
  return (
    <div className="p-10 w-full">
      ...
      <Separator className="bg-grey-font mt-4 mb-7" />
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea placeholder="Description" {...field} rows={3} onKeyDown={handleKeyPress} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <div className="flex gap-10">
            <Button type="submit" className="bg-pink-brown text-white">
              Submit
            </Button>
            <Button
              type="button"
              onClick={() => router.push("/categories")}
              className="bg-pink-brown text-white"
            >
              Discard
            </Button>
          </div>
